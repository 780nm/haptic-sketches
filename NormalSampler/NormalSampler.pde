  ...
  public void run(){
    ...
    if(haplyBoard.data_available()){
      ...
      && posEEAni.y >= 150 && posEEAni.y < 550) {
          color c = normals.get(int((posEEAni.x + 200)/(400./128.)), int((posEEAni.y - 150)/(400./128.)));
          float r = red(c);
          float b = blue(c);
          fEE.x = (r - 127);
          fEE.y = ((b - 127) * -1);
...
void update_animation(float th1, float th2, float xE, float yE){
  ...
  imageMode(CORNER);
  image(normals, deviceOrigin.x - 200, deviceOrigin.y + 150, 400, 400);
